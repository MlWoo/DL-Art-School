...
import torchvision
from modelflownet2.networks import Resample2d
...
class RecurrentImageGeneratorSequenceInjector(Injector):
    def __init__(self, opt, env):
        ...
        self.output_recurrent_index = (
            opt["output_recurrent_index"] if "output_recurrent_index" in opt.keys() else self.output_hq_index
        )
        ...
        self.first_inputs = opt["first_inputs"] if "first_inputs" in opt.keys() else opt["in"]
        self.do_backwards = opt["do_backwards"] if "do_backwards" in opt.keys() else True
        # When True, recurrent_index is not touched for the first iteration, allowing you to specify what is fed in.
        # When False, zeros are fed into the recurrent index.
        self.hq_recurrent = opt["hq_recurrent"] if "hq_recurrent" in opt.keys() else False
    ...
    def forward(self, state):
        ...
        for i in range(f):
            if first_step:
                ...
                with torch.no_grad() and autocast(enabled=False):
                    ...
                    assert hq_recurrent is not None, "hq_recurrent is not set in the first step"
        ...
        if self.hq_batched_output_key is not None:
            final_results[self.hq_batched_output_key] = (
                state["hq"].clone().permute(1, 0, 2, 3, 4).reshape(b * s, c, h, w)
            )
        for k, v in results.items():
            final_results[k] = torch.stack(v, dim=1)
            final_results[k + "_batched"] = torch.cat(
                v[:s], dim=0
    ...
    def produce_teco_visual_debugs(self, gen_input, gen_recurrent, it):
        if self.env["rank"] > 0:
            return
        base_path = osp.join(
            self.env["base_path"], "../../models", "visual_dbg", "teco_geninput", str(self.env["step"])
        )
...
def create_all_discriminator_sextuplets(input_list, lr_imgs, scale, total, flow_gen, resampler, margin):
    with autocast(enabled=False):
        ...
        flux_doubles_forward = [
            torch.stack([input_list[:, i], input_list[:, i + 1]], dim=2) for i in range(1, total + 1)
        ]
        flux_doubles_backward = [
            torch.stack([input_list[:, i], input_list[:, i - 1]], dim=2) for i in range(1, total + 1)
        ]
        ...
        for i in range(total):
            ...
            sext = torch.stack(
                [
                    ...
                    mid,
                    resampler(input_list[:, i + 2], flow_forward),
                ],
                dim=1,
            )
            # Apply margin
...
# 2) Uses the image flow generator (specified with 'image_flow_generator') to create detached flow fields
#   for the first and last images in the above sequence.
# 3) Warps the first and last images according to the flow field.
# 4) Composes the three base image and the 2 warped images and middle image into a tensor concatenated
#   at the filter dimension for both real and fake, resulting in a bx18xhxw shape tensor.
...
class TecoGanLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        # Per the tecogan paper, the GAN loss only pays attention to an inner part of the image
        # with the margin removed, to get rid of artifacts resulting from flow errors.
    ...
    def lowmem_forward(self, state):
        ...
        # Create a list of all the discriminator inputs, which will be reduced into the batch dim
    ...
    def fast_forward(self, state):
        ...
        # Create a list of all the discriminator inputs, which will be reduced into the batch dim
        # for efficient computation.
        combined_real_sext = create_all_discriminator_sextuplets(
            real, lr, self.scale, sequence_len - 2, flow_gen, self.resampler, self.margin
        )
        if self.gradient_penalty:
            combined_real_sext.requires_grad_()
        combined_fake_sext = create_all_discriminator_sextuplets(
            fake, lr, self.scale, sequence_len - 2, flow_gen, self.resampler, self.margin
        )
    ...
    def compute_loss(self, real_sext, fake_sext):
        ...
        if self.opt["gan_type"] in ["gan", "pixgan"]:
            ...
            l_step = self.criterion(d_real - torch.mean(d_fake), not self.for_generator) + self.criterion(
                d_fake_diff, self.for_generator
            )
        else:
    ...
    def produce_teco_visual_debugs(self, sext, lbl, it):
        if self.env["rank"] > 0:
            return
        base_path = osp.join(
            self.env["base_path"], "../../models", "visual_dbg", "teco_sext", str(self.env["step"]), lbl
        )
        os.makedirs(base_path, exist_ok=True)
...
class PingPongLoss(ConfigurableLoss):
    ...
    def produce_teco_visual_debugs(self, imglist):
        if self.env["rank"] > 0:
            return
        base_path = osp.join(
            self.env["base_path"], "../../models", "visual_dbg", "teco_pingpong", str(self.env["step"])
        )
        os.makedirs(base_path, exist_ok=True)
        cnt = imglist.shape[1]
        for i in range(cnt):
