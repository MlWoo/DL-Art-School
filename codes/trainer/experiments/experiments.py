...
class ModelParameterDepthTrackerMetrics(Experiment):
    ...
    def get_network_and_step_names(self):
        # Subclasses should return the network being debugged and the step name it is trained in.
        # return: (net, stepname)
        pass
    ...
    def before_step(self, opt, step_name, env, nets_to_train, pre_state):
        ...
        self.activate = (
            self.net in nets_to_train and step == step_name and self.step_num % opt["logger"]["print_freq"] == 0
        )
        if self.activate:
    ...
    def after_optimize(self, state):
        ...
        self.layer_means = []
        for params, cached_params in zip(self.params, self.cached_params):
            sum = torch.tensor(0)
            for param, cached_param in zip(params, cached_params):
                sum += torch.abs(cached_param - param.value().cpu())
            self.layer_means.append(sum / len(params))
