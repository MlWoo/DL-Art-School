...
class GradientDirectionOptimizer(BatchSizeOptimizer):
    ...
    def focus(self, model):
        if not hasattr(model, "_gradient_direction_optimizer_finished") or model._gradient_direction_optimizer_finished:
            all_params = list(
                filter(
                    lambda t: ".weight" in t[0] and not hasattr(t[1].requires_grad, "DO_NOT_TRAIN"),
                    list(model.named_parameters()),
                )
            )  # Extracts weight parameters. Who cares about biases anyways? :)
    ...
    def should_step(self, it):
        ...
        if model._gradient_direction_optimizer_prior_grads is not None:
            cur_dir = [
                self.vector_angle(lgrad, cgrad)
                for lgrad, cgrad in zip(model._gradient_direction_optimizer_prior_grads, cur_grads)
            ]
            delta_dir = [
                (cdir - ldir) for cdir, ldir in zip(cur_dir, model._gradient_direction_optimizer_prior_directions)
            ]
            model._gradient_direction_optimizer_prior_directions = cur_dir
            model._gradient_direction_optimizer_stopped_decreasing = [
                sd or dd < 0 for sd, dd in zip(model._gradient_direction_optimizer_stopped_decreasing, delta_dir)
            ]
            all_finished = all(model._gradient_direction_optimizer_stopped_decreasing)

            # For distributed optimizers, like ZeroRedundancyAdam,
            # we need to reach a consensus as to whether or not to reduce.
            ...
            if all_finished or model._gradient_direction_optimizer_step >= self.max_full_batches:
                ...
                # Fix the gradients. We've accumulated _gradient_direction_optimizer_step steps total,
                # so we need to divide the grads by that.
    ...
    def get_statistics(self):
        ...
        if self.last_number_iterations_filled:
            ...
            res["batch_size_opt_avg_iterations_per_step"] = (
                self.last_number_iterations[: self.last_number_iterations_i].mean().item()
            )
        return res
