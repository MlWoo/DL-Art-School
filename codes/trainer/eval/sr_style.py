...
class SrStyleTransferEvaluator(evaluator.Evaluator):
    def __init__(self, model, opt_eval, env):
        ...
        self.dataset = Stylegan2Dataset(
            {"path": self.fid_real_samples, "target_size": self.im_sz, "aug_prob": 0, "transparent": False}
        )
    ...
    def perform_eval(self):
        ...
        for batch in self.sampler:
            noise = (
                torch.FloatTensor(self.batch_sz, 3, self.im_sz, self.im_sz).uniform_(0.0, 1.0).to(self.env["device"])
            )
        ...
        return {"fid": fid_score.calculate_fid_given_paths([fid_real_path, fid_fake_path], self.batch_sz, True, 2048)}
