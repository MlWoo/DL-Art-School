...
from modelclip.contrastive_audio import ContrastiveAudio
from modeldiffusion.gaussian_diffusion import get_named_beta_schedule
from modeldiffusion.respace import SpacedDiffusion, space_timesteps
...
from utils.util import load_model_from_config, opt_get
...
class MusicDiffusionFid(evaluator.Evaluator):
    ...
    def __init__(self, model, opt_eval, env):
        ...
        diffusion_schedule = opt_get(
            env["opt"], ["steps", "generator", "injectors", "diffusion", "beta_schedule", "schedule_name"], None
        )
        ...
        self.diffuser = SpacedDiffusion(
            use_timesteps=space_timesteps(4000, [diffusion_steps]),
            model_mean_type="epsilon",
            model_var_type="learned_range",
            loss_type="mse",
            betas=get_named_beta_schedule(diffusion_schedule, 4000),
            conditioning_free=conditioning_free_diffusion_enabled,
            conditioning_free_k=conditioning_free_k,
        )
        self.spectral_diffuser = SpacedDiffusion(
            use_timesteps=space_timesteps(4000, [16 if self.ddim else 100]),
            model_mean_type="epsilon",
            model_var_type="learned_range",
            loss_type="mse",
        ...
        self.projector = ContrastiveAudio(
            model_dim=512, transformer_heads=8, dropout=0, encoder_depth=8, mel_channels=256
        )
        self.projector.load_state_dict(
            torch.load("../experiments/music_eval_projector.pth", map_location=torch.device("cpu"))
        )
        ...
        if mode == "spec_decode":
            ...
            self.cheater_decoder_diffuser = SpacedDiffusion(
                use_timesteps=space_timesteps(4000, [32]),
                model_mean_type="epsilon",
                model_var_type="learned_range",
                loss_type="mse",
                betas=get_named_beta_schedule("linear", 4000),
            ...
            self.spectral_diffuser = SpacedDiffusion(
                use_timesteps=space_timesteps(4000, [16]),
                model_mean_type="epsilon",
                model_var_type="learned_range",
                loss_type="mse",
                betas=get_named_beta_schedule("linear", 4000),
            ...
            self.kmeans_inj = KmeansQuantizerInjector(
                {"centroids": "../experiments/music_k_means_centroids.pth", "in": "in", "out": "out"}, {}
            )
            self.local_modules["ar_prior"] = get_ar_prior()
        elif "chained_sr" == mode:
            self.diffusion_fn = self.perform_chained_sr
        self.spec_fn = TorchMelSpectrogramInjector(
            {
                "n_mel_channels": 256,
                "mel_fmax": 11000,
                "filter_length": 16000,
    ...
    def perform_diffusion_spec_decode(self, audio, sample_rate=24000):
        real_resampled = audio
        audio = audio.unsqueeze(0)
        output_shape = (1, self.squeeze_ratio, audio.shape[-1] // self.squeeze_ratio)
        mel = self.spec_fn({"in": audio})["out"]
        gen = self.diffuser.p_sample_loop(self.model, output_shape, model_kwargs={"codes": mel})
        gen = pixel_shuffle_1d(gen, self.squeeze_ratio)

        return (
            gen,
            real_resampled,
            normalize_torch_mel(self.spec_fn({"in": gen})["out"]),
            normalize_torch_mel(mel),
            sample_rate,
            0,
        )

    def perform_diffusion_from_codes(self, audio, sample_rate=24000):
        real_resampled = audio
        audio = audio.unsqueeze(0)
        ...
        gen_mel = self.diffuser.p_sample_loop(
            self.model,
            mel_norm.shape,
        ...
        gen_wav = self.spectral_diffuser.p_sample_loop(
            self.spec_decoder, output_shape, model_kwargs={"aligned_conditioning": gen_mel_denorm}
        )
        gen_wav = pixel_shuffle_1d(gen_wav, 16)

        return gen_wav, real_resampled, gen_mel, mel_norm, sample_rate, torch.tensor([0])

    def perform_diffusion_from_codes_quant(self, audio, sample_rate=24000):
        audio = audio.unsqueeze(0)
        ...
        gen_wav = sampler(self.spec_decoder, output_shape, model_kwargs={"codes": gen_mel_denorm})
        gen_wav = pixel_shuffle_1d(gen_wav, 16)

        real_wav = sampler(self.spec_decoder, output_shape, model_kwargs={"codes": mel})
        real_wav = pixel_shuffle_1d(real_wav, 16)

        return gen_wav, real_wav.squeeze(0), gen_mel, mel_norm, sample_rate, torch.tensor([0])

    def perform_reconstruction_from_cheater_gen(self, audio, sample_rate=24000):
        audio = audio.unsqueeze(0)
        ...
        sampler = self.diffuser.ddim_sample_loop if self.ddim else self.diffuser.p_sample_loop
        # center-pad the conditioning input (the center isn't actually used).
        # this is hack for giving tfdpc5 a bigger working context.
        cheater_padded = torch.cat(
            [
                cheater[:, :, cheater.shape[-1] // 2 :],
                torch.zeros(1, 256, 160, device=cheater.device),
                cheater[:, :, : cheater.shape[-1] // 2],
            ],
            dim=-1,
        )
        gen_cheater = sampler(
            ...
            progress=True,
            causal=self.causal,
            causal_slope=self.causal_slope,
            model_kwargs={"conditioning_input": cheater_padded, "cond_start": 80},
        )

        # 2. Decode the cheater into a MEL
        gen_mel = self.cheater_decoder_diffuser.ddim_sample_loop(
            self.local_modules["cheater_decoder"].diff.to(audio.device),
            (1, 256, gen_cheater.shape[-1] * 16),
            progress=True,
        ...
        self.spec_decoder = self.spec_decoder.to(audio.device)
        gen_mel_denorm = denormalize_torch_mel(gen_mel)
        gen_wav = self.spectral_diffuser.ddim_sample_loop(
            self.spec_decoder, output_shape, model_kwargs={"codes": gen_mel_denorm}
        )
        gen_wav = pixel_shuffle_1d(gen_wav, 16)
    ...
    def perform_diffusion_from_codes_ar_prior(self, audio, sample_rate=24000):
        audio = audio.unsqueeze(0)
    ...
    def perform_chained_sr(self, audio, sample_rate=24000):
        audio = audio.unsqueeze(0)
        mel = self.spec_fn({"in": audio})["out"]
        mel_norm = normalize_torch_mel(mel)
        conditioning = mel_norm[:, :, :1200]
        downsampled = F.interpolate(mel_norm, scale_factor=1 / 4, mode="nearest")
        stage1_shape = (1, 256, downsampled.shape[-1] * 4)  # noqa: F841
        sampler = self.diffuser.ddim_sample_loop if self.ddim else self.diffuser.p_sample_loop
        ...
        stage2 = sampler(
            ...
            mel.shape,
            model_kwargs={
                "resolution": torch.tensor([0], device=audio.device),
                "x_prior": downsampled,
                "conditioning_input": conditioning,
            },
        )
        ...
        self.spec_decoder = self.spec_decoder.to(audio.device)
        gen_wav = self.spectral_diffuser.ddim_sample_loop(
            self.spec_decoder, output_shape, model_kwargs={"codes": stage2}
        )
        gen_wav = pixel_shuffle_1d(gen_wav, 16)
    ...
    def project(self, sample, sample_rate):
        sample = torchaudio.functional.resample(sample, sample_rate, 24000)
    ...
    def perform_eval(self):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(list(range(0, len(self.data), self.skip))):
                ...
                audio = load_audio(path, 24000).to(self.dev)
                if self.clip:
                    audio = audio[:, :100000]
                sample, ref, sample_mel, ref_mel, sample_rate, perplexity = self.diffusion_fn(audio)
                # Future note: need to normalize perplexity by the size of the input sample,
                # which are always equal for now but not gauranteed for the future.
                perplexities.append(perplexity)

                gen_projections.append(
                    self.project(sample, sample_rate).cpu()
                )  # Store on CPU to avoid wasting GPU memory.
                real_projections.append(self.project(ref, sample_rate).cpu())

                torchaudio.save(
                    os.path.join(save_path, f"{self.env['rank']}_{i}_gen.wav"), sample.squeeze(0).cpu(), sample_rate
                )
                torchaudio.save(os.path.join(save_path, f"{self.env['rank']}_{i}_real.wav"), ref.cpu(), sample_rate)
                torchvision.utils.save_image(
                    (sample_mel.unsqueeze(1) + 1) / 2, os.path.join(save_path, f"{self.env['rank']}_{i}_gen_mel.png")
                )
                torchvision.utils.save_image(
                    (ref_mel.unsqueeze(1) + 1) / 2, os.path.join(save_path, f"{self.env['rank']}_{i}_real_mel.png")
                )
            ...
            frechet_distance = torch.tensor(
                self.compute_frechet_distance(gen_projections, real_projections), device=self.env["device"]
            )
...
if __name__ == "__main__":
    ...
    diffusion = load_model_from_config(
        "X:\\dlas\\experiments\\train_music_diffusion_tfd_and_cheater.yml",
        "generator",
    ...
    opt_eval = {
        ...
        "diffusion_type": "from_codes_quant",
    }
