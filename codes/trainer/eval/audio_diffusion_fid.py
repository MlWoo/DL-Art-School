...
from modelaudio.tts.tacotron2 import text_to_sequence
from modelclip.mel_text_clip import MelTextCLIP
...
from utils.util import ceil_multiple, load_model_from_config, opt_get, pad_or_truncate
...
class AudioDiffusionFid(evaluator.Evaluator):
    """
    Evaluator produces generated audio from a diffusion model, then uses a CLIP model to judge the similarity between
    text & speech.
    ...
    def __init__(self, model, opt_eval, env):
        ...
        self.diffuser = load_discrete_vocoder_diffuser(
            desired_diffusion_steps=diffusion_steps,
        ...
        if mode == "tts":
            ...
            if mode == "tts9_mel_autoin":
                self.local_modules["autoregressive"] = (
    ...
    def perform_diffusion_tts(self, audio, codes, text, sample_rate=5500):
        real_resampled = torchaudio.functional.resample(audio, 24000, sample_rate).unsqueeze(0)
        ...
        gen = self.diffuser.p_sample_loop(
            self.model, output_shape, model_kwargs={"tokens": codes.unsqueeze(0), "conditioning_input": real_resampled}
        )
    ...
    def perform_original_diffusion_vocoder(self, audio, codes, text, sample_rate=11025):
        ...
        gen = self.diffuser.p_sample_loop(
            self.model,
            output_shape,
            model_kwargs={"spectrogram": back_to_mel, "conditioning_input": orig_audio.unsqueeze(0)},
        )
    ...
    def tts9_get_autoregressive_codes(self, mel, text):
        ...
        auto_latents = self.local_modules["autoregressive"](
            cond_inputs,
            text_codes,
    ...
    def perform_diffusion_tts9_mel_from_codes(self, audio, codes, text):
        ...
        gen_mel = self.diffuser.p_sample_loop(
            self.model,
            output_shape,
            model_kwargs={"aligned_conditioning": mel_codes, "conditioning_input": univnet_mel},
        )
        # denormalize mel
    ...
    def perform_diffusion_ctc(self, audio, codes, text):
        ...
        output_shape = univnet_mel.shape
        cond_mel = TorchMelSpectrogramInjector(
            {
                "n_mel_channels": 100,
                "mel_fmax": 11000,
                "filter_length": 8000,
                "normalize": True,
                "true_normalization": True,
                "in": "in",
                "out": "out",
            },
            {},
    ...
    def perform_diffusion_tfd(self, audio, codes, text):
        SAMPLE_RATE = 24000
        audio_resampled = torchaudio.functional.resample(audio, 24000, SAMPLE_RATE).unsqueeze(0)
        vmel = wav_to_mel(audio)
        umel = wav_to_univnet_mel(audio_resampled, do_normalization=True)
        gen_mel = self.diffuser.p_sample_loop(
            self.model,
            umel.shape,
            model_kwargs={"truth_mel": vmel, "conditioning_input": None, "disable_diversity": True},
        )
    ...
    def perform_diffusion_tfd_ar_prior(self, audio, codes, text):
        ...
        gen_mel = self.diffuser.p_sample_loop(self.model, umel.shape, model_kwargs={"codes": auto_latents})
    ...
    def load_projector(self):
        ...
        model = MelTextCLIP(
            ...
            speech_heads=8,
            speech_seq_len=1000,
            text_mask_percentage=0.15,
            voice_mask_percentage=0.15,
        )
    ...
    def project(self, projector, sample, sample_rate):
        ...
        return projector.get_speech_projection(mel).squeeze(
            0
        ...
        with torch.no_grad():
            ...
            for i in tqdm(list(range(0, len(self.data), self.skip))):
                ...
                gen_projections.append(
                    self.project(projector, sample, sample_rate).cpu()
                ...
                torchvision.utils.save_image(
                    (gen_mel.unsqueeze(1) + 1) / 2, os.path.join(save_path, f'{self.env["rank"]}_{i}_mel.png')
                )
                torchvision.utils.save_image(
                    (ref_mel.unsqueeze(1) + 1) / 2, os.path.join(save_path, f'{self.env["rank"]}_{i}_mel_target.png')
                )
                torchaudio.save(
                    os.path.join(save_path, f"{self.env['rank']}_{i}_gen.wav"), sample.squeeze(0).cpu(), sample_rate
                )
                torchaudio.save(
                    os.path.join(save_path, f"{self.env['rank']}_{i}_real.wav"), ref.squeeze(0).cpu(), sample_rate
                )
...
if __name__ == "__main__":
    diffusion = load_model_from_config(
        "X:\\dlas\\experiments\\train_tts_diffusion_tfd12_ar_inputs.yml",
        "generator",
        also_load_savepoint=False,
        load_path="X:\\dlas\\experiments\\train_tts_diffusion_tfd12_ar_inputs_pretrain\\models\\4500_generator.pth",
    ).cuda()
    opt_eval = {
        "eval_tsv": "Y:\\libritts\\test-clean\\transcribed-brief-w2v.tsv",
        "diffusion_steps": 50,
        "conditioning_free": False,
        "conditioning_free_k": 1,
        "diffusion_schedule": "linear",
        "diffusion_type": "tfd_ar",
    }
