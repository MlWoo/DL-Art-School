...
def find_registered_evaluators(base_path="trainer/eval"):
    ...
    for mod in module_iter:
        if mod.ispkg:
            ...
            # TODO: this causes an import error for PyCtcDecode.
            # get rid of this if there's a need to use that evaluator.
            if "eval_wer" in mod.name:
                continue
...
class CreateEvaluatorError(Exception):
    def __init__(self, name, available):
        super().__init__(f"Could not find the specified evaluator name: {name}.  Available evaluators:" f"{available}")
