...
class SrFidEvaluator(evaluator.Evaluator):
    ...
    def perform_eval(self):
        ...
        for batch in tqdm(self.dataloader):
            ...
            gen_lf = interpolate(
                interpolate(gen, scale_factor=1 / self.scale, mode="area"), scale_factor=self.scale, mode="nearest"
            )
            gen_hf = gen - gen_lf
        ...
        return {
            "sr_fid": fid_score.calculate_fid_given_paths(
                [self.fid_real_samples, fid_fake_path], self.batch_sz, True, 2048
            )
        }
