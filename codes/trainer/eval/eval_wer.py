...
from modelaudio.asr.w2v_wrapper import Wav2VecWrapper, only_letters
from modelaudio.tts.tacotron2 import sequence_to_text, tacotron_symbols
...
def fb_detokenize(seq):
    ...
    if fb_processor is None:
        fb_processor = Wav2Vec2Processor.from_pretrained("facebook/wav2vec2-large-960h")
    return fb_processor.decode(seq)


def perform_lm_processing(logits, decoder):
    from pyctcdecode.constants import DEFAULT_BEAM_WIDTH, DEFAULT_MIN_TOKEN_LOGP, DEFAULT_PRUNE_LOGP
...
if __name__ == "__main__":
    ...
    model = Wav2VecWrapper(
        vocab_size=148,
        basis_model="facebook/wav2vec2-large-robust-ft-libri-960h",
