...
def create_loss(opt_loss, env):
    ...
    if "teco_" in type:
        ...
        from modelimage_generation.stylegan import create_stylegan2_loss
        ...
        from modelaudio.tts.tacotron2 import Tacotron2Loss

        return Tacotron2Loss(opt_loss, env)
    elif type == "mel_gan":
        from modelaudio.vocoders.cdac.vqgan_loss import MelSpectrogramLoss

        return MelSpectrogramLoss(opt_loss, env)
    elif type == "stft_gan":
        from modelaudio.vocoders.cdac.vqgan_loss import MultiScaleSTFTLoss

        return MultiScaleSTFTLoss(opt_loss, env)
    else:
...
class Distillation(ConfigurableLoss):
    ...
    def forward(self, _, state):
        ...
        return self.loss(
            input=F.log_softmax(student / self.temperature, dim=-1),
            target=F.softmax(teacher / self.temperature, dim=-1),
        )
...
class PixLoss(ConfigurableLoss):
    ...
    def forward(self, _, state):
        ...
        with autocast(enabled=False):
            loss = self.criterion(fake.float(), real.float())
...
class SrPixLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        super().__init__(opt, env)
        self.opt = opt
        self.base_loss = opt_get(opt, ["base_loss"], 0.2)
    ...
    def forward(self, _, state):
        ...
        no_hf = F.interpolate(
            F.interpolate(real, scale_factor=1 / self.scale, mode="area"), scale_factor=self.scale, mode="nearest"
        )
...
class FeatureLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        self.netF = trainer.networks.define_F(
            which_model=opt["which_model_F"], load_path=opt["load_path"] if "load_path" in opt.keys() else None
        ).to(self.env["device"])
        if not env["opt"]["dist"]:
...
class InterpretedFeatureLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        self.netF_gen = trainer.networks.define_F(which_model=opt["which_model_F"], load_path=opt["load_path"]).to(
            self.env["device"]
        )
        if not env["opt"]["dist"]:
...
class GeneratorGanLoss(ConfigurableLoss):
    ...
    def forward(self, _, state):
        ...
        with autocast(enabled=self.env["opt"]["fp16"]):
            if self.opt["gan_type"] in ["gan", "pixgan", "pixgan_fea"]:
            ...
            elif self.opt["gan_type"] in ["audiogan_fea"]:
                ...
                l_feature = 0.0
                with autocast(enabled=False):
                    for x_fake in pred_g_fake:
                        l_fake = l_fake + self.criterion(x_fake[-1], True)

                    for i in range(len(pred_g_fake)):
                        for j in range(len(pred_g_fake[i]) - 1):
                            l_feature = l_feature + F.l1_loss(pred_g_fake[i][j], pred_d_real[i][j].detach())
                    self.metrics.append(("g_fake_loss", l_fake))
                    self.metrics.append(("g_feat_loss", l_feature))
                    return dict(gen=l_fake, feat=l_feature)
                ...
                loss = (
                    self.criterion(pred_d_real - torch.mean(pred_g_fake), False) + self.criterion(d_fake_diff, True)
                ) / 2
            else:
                raise NotImplementedError
        if self.min_loss != 0:
...
class DiscriminatorGanLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        if self.min_loss != 0:
            assert not self.env[
                "dist"
            ]  # distributed training does not support 'min_loss' - it can result in backward() desync by design.
            self.loss_rotating_buffer = torch.zeros(10, requires_grad=False)
            self.rb_ptr = 0
            self.losses_computed = 0

    def forward(self, net, state):
        ori_real = extract_params_from_state(self.opt["real"], state)
        real = [r.detach() for r in ori_real]
        if self.gradient_penalty:
            [r.requires_grad_() for r in real]
        ori_fake = extract_params_from_state(self.opt["fake"], state)
        new_state = {}
        fake = [f.detach() for f in ori_fake]
        new_state = {}
        if self.noise:
            nreal = []
            nfake = []
        ...
        if self.opt["gan_type"] in ["gan", "pixgan"]:
            ...
            self.metrics.append(("d_fake_diff", torch.mean(d_fake_diff)))
            loss = self.criterion(d_real - torch.mean(d_fake), True) + self.criterion(d_fake_diff, False)
        elif self.opt["gan_type"] == "audiogan_fea":
            with autocast(enabled=False):
                l_fake = 0.0
                l_real = 0.0
                d_fake_scores = 0.0
                ...
                for x_fake, x_real in zip(d_fake, d_real):
                    l_fake = l_fake + self.criterion(x_fake[-1].float(), False)
                    d_fake_scores += torch.mean(x_fake[-1])
                    l_real = l_real + self.criterion(x_real[-1].float(), True)
                    d_real_scores += torch.mean(x_real[-1])
                self.metrics.append(("d_fake_score", d_fake_scores))
                self.metrics.append(("d_real_score", d_real_scores))
                self.metrics.append(("d_fake_loss", l_fake))
                self.metrics.append(("d_real_loss", l_real))
                l_total = l_real + l_fake
                loss = l_total
        else:
            raise NotImplementedError
        ...
        if self.gradient_penalty:
            ...
            from modelimage_generation.stylegan.stylegan2_lucidrains import gradient_penalty
            ...
            # The gp_structure is a useful visual debugging tool to see what areas of the generated image
            # the disc is paying attention to.
            gpimg = (
                (gp_structure / (torch.std(gp_structure, dim=(-1, -2), keepdim=True) * 2))
                - torch.mean(gp_structure, dim=(-1, -2), keepdim=True)
                + 0.5
            )
...
class GeometricSimilarityGeneratorLoss(ConfigurableLoss):
    ...
    def random_alteration(self):
        return random.choice(
            [
                (functools.partial(torch.flip, dims=(2,)), functools.partial(torch.flip, dims=(2,))),
    ...
    def forward(self, net, state):
        ...
        if self.opt["criterion"] == "cosine":
            return self.criterion(
                state[self.opt["real"]], upsampled_altered, torch.ones(1, device=upsampled_altered.device)
            )
        else:
...
class TranslationInvarianceLoss(ConfigurableLoss):
    ...
    def forward(self, net, state):
        ...
        translation = random.choice(
            [
                ("top_right", border_sz, border_sz + self.overlap, 0, self.overlap),
        ...
        real_shared_output = gen_output[
            :, :, border_sz : border_sz + self.overlap, border_sz : border_sz + self.overlap
        ]

        if self.opt["criterion"] == "cosine":
            return self.criterion(
                fake_shared_output, real_shared_output, torch.ones(1, device=real_shared_output.device)
            )
        else:
...
class RecursiveInvarianceLoss(ConfigurableLoss):
    def __init__(self, opt, env):
        ...
        self.gen_input_for_alteration = opt["input_alteration_index"] if "input_alteration_index" in opt.keys() else 0
        self.gen_output_to_use = opt["generator_output_index"] if "generator_output_index" in opt.keys() else None
        self.recursive_depth = opt[
            "recursive_depth"
        ]  # How many times to recursively feed the output of the generator back into itself
        self.downsample_factor = opt[
            "downsample_factor"
        ]  # Just 1/opt['scale']. Necessary since this loss doesnt have access to opt['scale'].
    ...
    def forward(self, net, state):
        ...
        for i in range(self.recursive_depth):
            input[self.gen_input_for_alteration] = torch.nn.functional.interpolate(
                recurrent_gen_output, scale_factor=self.downsample_factor, mode="nearest"
            )
            with autocast(enabled=self.env["opt"]["fp16"]):
