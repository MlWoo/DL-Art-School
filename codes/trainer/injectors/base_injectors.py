...
from trainer.weight_scheduler import get_scheduler_for_opt
...
class GeneratorInjector(Injector):
    def __init__(self, opt, env):
        ...
        env_half_type = opt_get(self.env["opt"], ["half_type"], None)
        self_half_type = opt_get(opt, ["half_type"], None)

        if self_half_type is not None:
            half_type = self_half_type
        elif env_half_type is not None:
            half_type = env_half_type
        else:
            half_type = "fp16"

        def get_half_type(half_type):
            if half_type == "fp16":
        ...
        self.half_type = get_half_type(half_type)
    ...
    def forward(self, state):
        ...
        enabled = self.env["opt"]["fp16"] and self.fp16_override
        dtype = self.half_type if enabled else torch.float32
        ...
        if isinstance(self.output, list):
            ...
            # IF YOU REACH THIS ERROR, REMOVE THE BRACES AROUND YOUR OUTPUTS IN THE YAML CONFIG
...
class DiscriminatorInjector(Injector):
    ...
    def forward(self, state):
        with torch.amp.autocast("cuda", enabled=self.env["opt"]["fp16"]):
...
class InterpolateInjector(Injector):
    ...
    def forward(self, state):
        scaled = torch.nn.functional.interpolate(
            state[self.opt["in"]], scale_factor=self.opt["scale_factor"], size=self.opt["size"], mode=self.opt["mode"]
        )
        return {self.opt["out"]: scaled}


# Extracts four patches from the input image, each a square of 'patch_size'.
# The input images are taken from each of the four corners of the image.
# The intent of this loss is that each patch shares some part of the input, which
# can then be used in the translation invariance loss.
#
# This injector is unique in that it does not only produce the specified output label into state.
# Instead it produces five outputs for the specified label, one for each corner of the input as well as
# the specified output, which is the top left corner. See the code below to find out how this works.
#
# Another note: this injector operates differently in eval mode (e.g. when env['training']=False)
# - in this case, it simply sets all the output state variables to the input.
# This is so that you can feed the output of this injector directly into your generator in training
...
class MarginRemoval(Injector):
    ...
    def forward(self, state):
        ...
        if self.random_shift_max > 0:
            ...
            # If it works at all, I should reconsider using Resample2D, for example.
...
class RandomCropInjector(Injector):
    def __init__(self, opt, env):
        ...
        self.operator = RandomResizedCrop(
            ...
            scale=(scale, 1),
            ratio=(0.99, 1),  # An aspect ratio range is required, but .99,1 is effectively "none".
            resample="NEAREST",
        )
