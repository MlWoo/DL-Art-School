...
class UNetONNXWrapper(torch.nn.Module):
    ...
    def forward(self, sample, timestep, encoder_hidden_states, text_embeds, time_ids):
        ...
        return self.unet_model(
            sample, timestep, encoder_hidden_states=encoder_hidden_states, added_cond_kwargs=added_cond_kwargs
        ).sample  # 通常返回 .sample
...
def export_unet_to_onnx(
    ...
    dummy_sample = torch.randn(
        opt_export_batch_size, latent_channels, latent_height, latent_width, dtype=torch_dtype
    ).cuda()
    ...
    dummy_encoder_hidden_states = torch.randn(
        opt_export_batch_size, text_seq_len, encoder_hidden_dim, dtype=torch_dtype
    ).cuda()
    ...
    try:
        ...
        onnx.save_model(
            ...
            location=external_weights_filename,
            size_threshold=1024,
        )
...
def convert_unet_onnx_to_trt(
    ...
    with open(onnx_path, "rb") as model_file:
        if not parser.parse(model_file.read()):
            ...
            for err_idx in range(parser.num_errors):
                print(f"  Parser error: {parser.get_error(err_idx)}")
            return False
    ...
    profile.set_shape("timestep", min=(profile_min_batch,), opt=(profile_opt_batch,), max=(profile_max_batch,))
    profile.set_shape(
    ...
    profile.set_shape(
        "time_ids",  # Assuming 3D for time_ids as per previous error
    ...
    print(
        f"  UNet Optimization profile added. Batch (min/opt/max): ({profile_min_batch}/{profile_opt_batch}/{profile_max_batch})"
    )

    print("  Building TensorRT UNet engine... This may take a significant time.")
    plan = builder.build_serialized_network(network, config)
    if plan is None:
        print("ERROR: Failed to build the TensorRT UNet engine.")
        return False
...
if __name__ == "__main__":
    ...
    if unet is None:
        print("ERROR: UNet model could not be loaded.")
        exit(1)
    ...
    if not os.path.exists(FINAL_ONNX_UNET_PATH):
        ...
        if not actual_final_onnx_unet_path:
            print("ERROR: ONNX UNet export failed.")
            exit(1)
    else:
    ...
    if actual_final_onnx_unet_path and os.path.exists(actual_final_onnx_unet_path):
        if not os.path.exists(TRT_UNET_PATH):
            ...
            if not success:
                print("ERROR: TensorRT UNet conversion failed.")
                exit(1)
        else:
