...
import tensorrt as trt
import torch
from diffusers import LMSDiscreteScheduler  # Or your chosen scheduler
from diffusers import AutoencoderKL, EulerDiscreteScheduler, PNDMScheduler, StableDiffusionXLInpaintPipeline
from PIL import Image
    ...
    if has_nan or has_inf:
        ...
        if raise_error:
            if has_nan and has_inf:
                raise ValueError(f"NaN and Inf detected in {name} {step_info}")
            if has_nan:
                raise ValueError(f"NaN detected in {name} {step_info}")
            if has_inf:
                raise ValueError(f"Inf detected in {name} {step_info}")
        return True
...
class TRTEngine:
    ...
    def _prepare_binding_info(self):
        ...
        if not hasattr(self.engine, "num_io_tensors"):
            raise AttributeError(
                f"'ICudaEngine' object (version {trt.__version__}) has no attribute 'num_io_tensors'. This is critical. Please check TensorRT installation."
            )
    ...
    def __call__(self, inputs_dict: dict):
        ...
        for name, tensor in inputs_dict.items():
            ...
            if name not in self.tensor_name_to_idx_map:
                raise ValueError(
                    f"Tensor name '{name}' not found in prepared binding map. This should not happen if _prepare_binding_info was successful."
                )
            idx = self.tensor_name_to_idx_map[name]
            ...
            if tensor.dtype != expected_torch_dtype:
                print(
                    f"Warning: Input tensor '{name}' dtype mismatch. Expected {expected_torch_dtype}, got {tensor.dtype}. Casting."
                )
                tensor = tensor.to(expected_torch_dtype)
        ...
        for name in self.output_names:
            if name not in self.tensor_name_to_idx_map:
                raise ValueError(
                    f"Tensor name '{name}' not found in prepared binding map for outputs. This should not happen if _prepare_binding_info was successful."
                )
            idx = self.tensor_name_to_idx_map[name]
            ...
            try:
                ...
                if -1 not in output_shape_from_context:
                    ...
                    print(
                        f"Warning: Output tensor '{name}' shape from context.get_binding_shape is still dynamic: {output_shape_from_context}. Will use fallback."
                    )
            except AttributeError:
                print(
                    f"Warning: context.get_binding_shape not available on this TRT version for output '{name}'. Using fallback."
                )
            except Exception as e:
                print(f"Error calling context.get_binding_shape for output '{name}': {e}. Using fallback.")
                ...
                if name == "sample":
                    if len(build_time_opt_shape) == 4:
                        if build_time_opt_shape[1] == -1:
                            print(
                                f"Info: Output '{name}', dim 1 (Channels) was -1. Setting to global IMAGE_CHANNELS: {IMAGE_CHANNELS}"
                            )
                            build_time_opt_shape[1] = IMAGE_CHANNELS
                        if build_time_opt_shape[2] == -1:
                            print(
                                f"Info: Output '{name}', dim 2 (Height) was -1. Setting to global IMAGE_HEIGHT: {IMAGE_HEIGHT}"
                            )
                            build_time_opt_shape[2] = IMAGE_HEIGHT
                        if build_time_opt_shape[3] == -1:
                            print(
                                f"Info: Output '{name}', dim 3 (Width) was -1. Setting to global IMAGE_WIDTH: {IMAGE_WIDTH}"
                            )
                            build_time_opt_shape[3] = IMAGE_WIDTH
                    ...
                    if len(build_time_opt_shape) == 4:
                        if build_time_opt_shape[1] == -1:
                            print(
                                f"Info: Output '{name}', dim 1 (Channels) was -1. Setting to global UNET_OUTPUT_CHANNELS: {UNET_OUTPUT_CHANNELS}"
                            )
                            build_time_opt_shape[1] = UNET_OUTPUT_CHANNELS
                        if build_time_opt_shape[2] == -1:
                            latent_height = IMAGE_HEIGHT // self.vae_scale_factor
                            print(
                                f"Info: Output '{name}', dim 2 (Latent Height) was -1. Setting to derived latent_height: {latent_height}"
                            )
                            build_time_opt_shape[2] = latent_height
                        if build_time_opt_shape[3] == -1:
                            latent_width = IMAGE_WIDTH // self.vae_scale_factor
                            print(
                                f"Info: Output '{name}', dim 3 (Latent Width) was -1. Setting to derived latent_width: {latent_width}"
                            )
                            build_time_opt_shape[3] = latent_width
                else:
                    for i_dim in range(1, len(build_time_opt_shape)):
                        if build_time_opt_shape[i_dim] == -1:
                            print(
                                f"CRITICAL WARNING: Dimension {i_dim} for output '{name}' is -1 in build_time_opt_shape and unresolved. Using 1 as placeholder. THIS IS RISKY."
                            )
                            build_time_opt_shape[i_dim] = 1
        ...
        for original_io_index, ptr in binding_idx_to_ptr.items():
            if original_io_index >= len(final_bindings_list):
                raise IndexError(
                    f"Binding index {original_io_index} (derived from I/O index) is out of range for bindings list of size {len(final_bindings_list)}."
                )
            final_bindings_list[original_io_index] = ptr
            ...
            if final_bindings_list[i] == 0:
                ...
                for name, mapped_idx in self.tensor_name_to_idx_map.items():
                    if mapped_idx == i:
                        ...
                        break
                raise RuntimeError(
                    f"Binding for index {i} (tensor: {missing_tensor_name or 'Unknown'}) was not set. Ensure all engine inputs/outputs are handled."
                )
            ...
            try:
                for i, ptr in enumerate(final_bindings_list):
                    if hasattr(self.context, "set_binding_address"):
                        if not self.context.set_binding_address(i, ptr):
                            raise RuntimeError(
                                f"Failed to set binding address for binding index {i} using set_binding_address."
                            )
                    elif hasattr(self.context, "set_tensor_address"):
                        tensor_name_for_idx_i = None
                        for t_name, t_idx in self.tensor_name_to_idx_map.items():
                        ...
                        if tensor_name_for_idx_i is None:
                            raise RuntimeError(
                                f"Could not find tensor name for binding index {i} to use with set_tensor_address."
                            )
                        if not self.context.set_tensor_address(tensor_name_for_idx_i, ptr):
                            raise RuntimeError(
                                f"Failed to set tensor address for tensor '{tensor_name_for_idx_i}' (binding index {i}) using set_tensor_address."
                            )
                    else:
                        raise AttributeError(
                            "IExecutionContext has execute_async_v3 but is missing set_binding_address and set_tensor_address. Cannot set bindings for v3."
                        )
                ...
                if "incompatible function arguments" in str(e_v3_type) and "bindings" in str(e_v3_type):
                    print(
                        f"DEBUG: execute_async_v3 was called with 'bindings' kwarg due to previous logic, but signature is (self, stream_handle). Error: {e_v3_type}"
                    )
                else:
                    print(f"DEBUG: execute_async_v3 path failed with TypeError: {e_v3_type}")
            except Exception as e_v3:
        ...
        if not executed_successfully and hasattr(self.context, "execute_async_v2"):
            try:
                ...
                if "execute_async_v3" in str(e_v2):
                    print(
                        f"DEBUG: execute_async_v2 failed suggesting v3 ({e_v2}), but v3 path was already tried or failed. This is a problematic state."
                    )
                else:
                    print(f"DEBUG: execute_async_v2 failed for a different reason: {e_v2}")
            except Exception as e_v2_other:
        ...
        if not executed_successfully and hasattr(self.context, "execute_async"):
            try:
                ...
                self.context.execute_async(
                    batch_size=current_batch_size, bindings=final_bindings_list, stream_handle=stream_handle
                )
                executed_successfully = True
            except Exception as e_base:
                print(f"DEBUG: execute_async (base) failed: {e_base}")
        ...
        if not executed_successfully:
            raise AttributeError(
                "No suitable execute_async method (v3, v2, or base) was found or executed successfully on IExecutionContext."
            )
...
def encode_text(prompt_text, tokenizer, text_encoder_model):
    text_inputs = tokenizer(
        prompt_text, padding="max_length", max_length=tokenizer.model_max_length, truncation=True, return_tensors="pt"
    )
    with torch.no_grad():
        prompt_embeds = text_encoder_model(text_inputs.input_ids.to(text_encoder_model.device))
        check_tensor(
            prompt_embeds.last_hidden_state, f"TextEncoder Output last_hidden_state for '{prompt_text[:20]}...'"
        )

    if not hasattr(prompt_embeds, "hidden_states") or prompt_embeds.hidden_states is None:
        print(
            "Warning: 'hidden_states' not found in text encoder output. Using 'last_hidden_state' for sequence embeddings."
        )
        seq_embeds = prompt_embeds.last_hidden_state
    else:
    ...
    if isinstance(text_encoder_model, CLIPTextModelWithProjection):
        if hasattr(prompt_embeds, "text_embeds") and prompt_embeds.text_embeds is not None:
            ...
            print(
                "Warning: 'text_embeds' (projected) not found on CLIPTextModelWithProjection output, using 'pooler_output'."
            )
            pooled_embeds = prompt_embeds.pooler_output
        else:
            print(
                "Warning: Neither 'text_embeds' (projected) nor 'pooler_output' found on CLIPTextModelWithProjection output. Using last_hidden_state[:, 0]."
            )
            pooled_embeds = prompt_embeds.last_hidden_state[:, 0]
    elif isinstance(text_encoder_model, CLIPTextModel):
        if hasattr(prompt_embeds, "pooler_output") and prompt_embeds.pooler_output is not None:
        ...
        print(
            f"Warning: Unknown text_encoder_model type ({type(text_encoder_model)}). Attempting to use 'pooler_output' or fallback."
        )
        if hasattr(prompt_embeds, "pooler_output") and prompt_embeds.pooler_output is not None:
            pooled_embeds = prompt_embeds.pooler_output
        elif hasattr(prompt_embeds, "last_hidden_state"):
            ...
            raise AttributeError(
                f"Cannot determine pooled embeddings for text_encoder_model of type {type(text_encoder_model)}"
            )
...
if __name__ == "__main__":
    ...
    text_encoder_1 = (
        CLIPTextModel.from_pretrained(
            ORIGINAL_MODEL_ID, subfolder="text_encoder", torch_dtype=DTYPE, use_safetensors=True
        )
        .cuda()
        .eval()
    )
    text_encoder_2 = (
        CLIPTextModelWithProjection.from_pretrained(
            ORIGINAL_MODEL_ID, subfolder="text_encoder_2", torch_dtype=DTYPE, use_safetensors=True
        )
        .cuda()
        .eval()
    )
    vae_pt = (
        AutoencoderKL.from_pretrained(ORIGINAL_MODEL_ID, subfolder="vae", torch_dtype=DTYPE, use_safetensors=True)
        .cuda()
        .eval()
    )
    ...
    mask_latents = torch.nn.functional.interpolate(mask_pt, size=(init_latents.shape[2], init_latents.shape[3]))
    masked_image_latents = init_latents * (1 - mask_latents)
    ...
    if t_start_idx_numpy.size == 0:
        print(
            f"Warning: init_timestep {init_timestep.item()} not found in scheduler.timesteps. Starting from the beginning of timesteps."
        )
        t_start = 0
    else:
        t_start = t_start_idx_numpy[0]
    ...
    for i, t in enumerate(timesteps_to_iterate):
        ...
        latent_model_input_unscaled = torch.cat([latents, mask_latents, masked_image_latents], dim=1)
        check_tensor(
            latent_model_input_unscaled, "latent_model_input_unscaled", step_info=current_step_info, raise_error=True
        )
