...
def export_unet_to_onnx(pipeline, onnx_path):
    ...
    print(f"Shape of encoder_hidden_states_val from encode_prompt: {encoder_hidden_states_val.shape}") # e.g., (B, 77, 2048)
    ...
    if hasattr(pipeline.text_encoder_2, "config") and hasattr(pipeline.text_encoder_2.config, "projection_dim"):
        ...
        print(f"Derived text_encoder_projection_dim_val from text_encoder_2.config.projection_dim: {text_encoder_projection_dim_val}")

    if text_encoder_projection_dim_val is None and hasattr(pipeline.unet, "config") and hasattr(pipeline.unet.config, "projection_class_embeddings_input_dim"):
        ...
        print(f"Derived text_encoder_projection_dim_val from unet.config.projection_class_embeddings_input_dim: {text_encoder_projection_dim_val}")
    ...
    if text_encoder_projection_dim_val is None:
        ...
        print(f"Warning: Could not reliably derive text_encoder_projection_dim. Using default: {default_sdxl_projection_dim}. This might be incorrect for your specific model.")
    ...
    if not hasattr(pipeline.unet.config, "addition_time_embed_dim") or \
       not isinstance(pipeline.unet.config.addition_time_embed_dim, int):
        raise ValueError(
    ...
    time_ids_val_maybe_batched_for_cfg = torch.cat(time_ids_val_maybe_batched_for_cfg, dim=0).to(device="cuda", dtype=torch.float16)
    ...
    print(f"Shape of time_ids_val from _get_add_time_ids (before potential slicing): {time_ids_val_maybe_batched_for_cfg.shape}")
    ...
    print(f"Final shape of time_ids_val to be used for UNet: {time_ids_val.shape}") # Should be (BATCH_SIZE, 6) for SDXL
    ...
    try:
        ...
        try:
            ...
            print("Proceeding with conversion to external data format, which might resolve size issues for the protobuf itself.")
        ...
        onnx_model_loaded = onnx.load(initial_onnx_export_path, load_external_data=False) # Initially, don't load external data if any (though torch.onnx.export doesn't create it by default)
        ...
        print(f"Saving ONNX model with external data. Graph: {final_onnx_path_for_trt}, Weights: {external_weights_filename}")
...
def convert_onnx_to_trt(onnx_path, trt_engine_path, min_batch=1, opt_batch=1, max_batch=1, min_latent_h=64, opt_latent_h=128, max_latent_h=128, min_latent_w=64, opt_latent_w=128, max_latent_w=128, min_seq_len=77, opt_seq_len=77, max_seq_len=77, fp16_mode=True, workspace_size_gb=1):
    ...
    profile.set_shape(
        "time_ids",
        (min_batch, added_time_ids_dim),
        (opt_batch, added_time_ids_dim),
        (max_batch, added_time_ids_dim)
    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(final_onnx_path_for_trt): # Check for the final, TRT-ready ONNX file
        ...
        if returned_onnx_path and returned_onnx_path == final_onnx_path_for_trt:
            ...
            print(f"Warning: Exported ONNX path {returned_onnx_path} differs from expected {final_onnx_path_for_trt}. Using returned path.")
    ...
    if os.path.exists(final_onnx_path_for_trt) and not os.path.exists(TRT_UNET_PATH):
        ...
        convert_onnx_to_trt(
            ...
            min_batch=BATCH_SIZE, opt_batch=BATCH_SIZE, max_batch=BATCH_SIZE, # Example
            min_latent_h=IMAGE_HEIGHT // pipe.vae_scale_factor, opt_latent_h=IMAGE_HEIGHT // pipe.vae_scale_factor, max_latent_h=IMAGE_HEIGHT // pipe.vae_scale_factor,
            min_latent_w=IMAGE_WIDTH // pipe.vae_scale_factor, opt_latent_w=IMAGE_WIDTH // pipe.vae_scale_factor, max_latent_w=IMAGE_WIDTH // pipe.vae_scale_factor,
            min_seq_len=encoder_hidden_states_val.shape[1] if 'encoder_hidden_states_val' in locals() else 77, # Use actual if available
