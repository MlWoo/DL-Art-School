...
from modelaudio.module.wenet.class_utils import (
...
from modelaudio.module.wenet.convolution import ConvolutionModule
from modelaudio.module.wenet.mask import add_optional_chunk_mask, make_pad_mask
...
class ConformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.norm_ff = WENET_NORM_CLASSES[layer_norm_type](size, eps=norm_eps)  # for the FNN module
        self.norm_mha = WENET_NORM_CLASSES[layer_norm_type](size, eps=norm_eps)  # for the MHA module
        ...
        if self.conv_module is not None:
            self.norm_conv = WENET_NORM_CLASSES[layer_norm_type](size, eps=norm_eps)  # for the CNN module
...
class BaseEncoder(torch.nn.Module):
    def __init__(
        ...
        Args:
            ...
            key_bias: whether use bias in attention.linear_k, False for whisper model
        ...
        self.embed = WENET_SUBSAMPLE_CLASSES[input_layer](
            ...
            (
                ...
                else pos_emb_class(output_size, output_size // attention_heads, positional_dropout_rate)
            ),
        )
    ...
    def forward(
        ...
        #   mask_pad)
        # else:
        xs_pre, xs = self.forward_layers(xs, chunk_masks, pos_emb, mask_pad)
        if self.normalize_before:
    ...
    def forward_layers(
        ...
        if self.pre_conformer_layers is not None:
            cache = torch.zeros((0, 0, 0), device=xs.device)
            for layer in self.pre_conformer_layers:
                xs, _ = layer(xs, mask_pad, cache)

        for layer in self.encoders1:
            xs, chunk_masks, _, _ = layer(xs, chunk_masks, pos_emb, mask_pad)

        xs1 = xs
        for layer in self.encoders2:
            xs, chunk_masks, _, _ = layer(xs, chunk_masks, pos_emb, mask_pad)
        xs2 = xs
        return xs1, xs2
...
class ConformerEncoder(BaseEncoder):
    ...
    def __init__(
        ...
        num_input_dim: Optional[int] = None,
        output_size: int = 256,
        attention_heads: int = 4,
        linear_units: int = 2048,
        pre_layers: int = 3,
        ...
        if pre_layers > 0:
            self.pre_conformer_layers = torch.nn.ModuleList(
                [ConvolutionModule(*convolution_layer_args) for _ in range(pre_layers)]
            )
        else:
            self.pre_conformer_layers = None

        self.encoders1 = torch.nn.ModuleList(
            [
                ConformerEncoderLayer(
                    output_size,
                    WENET_ATTENTION_CLASSES[selfattention_layer_type](*encoder_selfattn_layer_args),
        ...
        self.encoders2 = torch.nn.ModuleList(
            [
                ConformerEncoderLayer(
                    output_size,
                    WENET_ATTENTION_CLASSES[selfattention_layer_type](*encoder_selfattn_layer_args),
        ...
        self.channels_up = output_size
        self.reduction_steps = 4


if __name__ == "__main__":
    x = torch.randn(16, 16000, 128)
    ...
    y, _ = model(x, spec_length, decoding_chunk_size=4)
