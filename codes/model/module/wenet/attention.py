...
import torch
from modelaudio.module.wenet.rope_utils import llama_apply_rotary_emb
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward_attention(
        ...
        #   2. jit (16/-1, -1/-1, 16/0, 16/4)
            ...
            attn = torch.softmax(scores.float(), dim=-1).type_as(value)  # (batch, ..., head, time1, time2)
    ...
    def _update_kv_and_cache(
        ...
        if self.h_kv != self.h and self.h_kv != 1:
            ...
            k = (
                k.unsqueeze(-3)
                .expand(k_shape[:-2] + torch.Size([n_repeat]) + k_shape[-2:])
                .reshape(k_shape[:-3] + torch.Size([self.h_kv * n_repeat]) + k_shape[-2:])
            )
            v_shape = v.size()
            v = (
                v.unsqueeze(-3)
                .expand(v_shape[:-2] + torch.Size([n_repeat]) + v_shape[-2:])
                .reshape(v_shape[:-3] + torch.Size([self.h_kv * n_repeat]) + v_shape[-2:])
            )
    ...
    def forward(
        ...
        if not self.use_sdpa:
            ...
            output = (
                output.transpose(1, 2).contiguous().view(query.size(0), -1, self.h * self.d_k)
            )  # (batch, time1, d_model)
