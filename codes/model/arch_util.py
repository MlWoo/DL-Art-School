...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None, factor=2):
        ...
        stride = factor
        if use_conv:
            self.op = conv_nd(dims, self.channels, self.out_channels, ksize, stride=stride, padding=pad)
        else:
...
def build_local_attention_mask(n, m, fixed_region=0):
    """
    ...
    Args:
        ...
        m: Size of local context (uni-directional, e.g. the total context is l*2)
    ...
    """
    assert m * 2 < n, f"Local context must be less than global context. {m}, {n}"
    o = torch.arange(0, n)
    c = o.unsqueeze(-1).repeat(1, n)
    r = o.unsqueeze(0).repeat(n, 1)
    localized = ((-(r - c).abs()) + m).clamp(0, m - 1) / (m - 1)
...
class RelativeQKBias(nn.Module):
    ...
    def __init__(self, m, max_positions=4000, symmetric=True):
        super().__init__()
        if symmetric:
            self.emb = nn.Parameter(torch.randn(m + 1) * 0.01)
            o = torch.arange(0, max_positions)
            c = o.unsqueeze(-1).repeat(1, max_positions)
            r = o.unsqueeze(0).repeat(max_positions, 1)
            M = ((-(r - c).abs()) + m).clamp(0, m)
        else:
            self.emb = nn.Parameter(torch.randn(m * 2 + 2) * 0.01)
            a = torch.arange(0, max_positions)
            c = a.unsqueeze(-1) - a
            m = (c >= -m).logical_and(c <= m)
            M = (m + c + 1) * m
    ...
    def forward(self, n):
        ...
        # However, indexing operations like this have horrible efficiency on GPUs:
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(
                    m.weight, a=0.1, mode="fan_out", nonlinearity="leaky_relu" if self.lelu else "linear"
                )
                m.weight.data *= weight_init_factor
                if m.bias is not None:
                    m.bias.data.zero_()
...
class MultiConvBlock(nn.Module):
    def __init__(
        self, filters_in, filters_mid, filters_out, kernel_size, depth, scale_init=1, norm=False, weight_init_factor=1
    ):
        assert depth >= 2
        ...
        self.bnconvs = nn.ModuleList(
            [
                ConvBnLelu(
                    filters_in, filters_mid, kernel_size, norm=norm, bias=False, weight_init_factor=weight_init_factor
                )
            ]
            + [
                ConvBnLelu(
                    filters_mid, filters_mid, kernel_size, norm=norm, bias=False, weight_init_factor=weight_init_factor
                )
...
class ConjoinBlock(nn.Module):
    def __init__(self, filters_in, filters_out=None, filters_pt=None, block=ConvGnSilu, norm=True):
        ...
        self.process = block(
            filters_in + filters_pt, filters_in + filters_pt, kernel_size=3, bias=False, activation=True, norm=norm
        )
        self.decimate = block(
            filters_in + filters_pt, filters_out, kernel_size=1, bias=False, activation=False, norm=norm
        )
...
class ReferenceJoinBlock(nn.Module):
    def __init__(
        self, nf, residual_weight_init_factor=1, block=ConvGnLelu, final_norm=False, kernel_size=3, depth=3, join=True
    ):
