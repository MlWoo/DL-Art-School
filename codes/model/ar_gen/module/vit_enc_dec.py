...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, video_size=224, video_length=16, patch_size=16, patch_length=1, in_chans=3, embed_dim=768):
        ...
        num_patches = (
            (video_length // patch_length) * (video_size[1] // patch_size[1]) * (video_size[0] // patch_size[0])
        )
...
class ViTEncoder(nn.Module):
    ...
    def __init__(
        ...
        input_info: Union[VideoInputConfig, AudioInputConfig] = VideoInputConfig(
            video_size=256,
            video_length=16,
            patch_size=8,
            patch_length=4,
        ...
        if isinstance(input_info, VideoInputConfig):
            self.patch_embed = PatchEmbed(
                video_size=input_info.video_size,
                video_length=input_info.video_length,
                patch_size=input_info.patch_size,
                patch_length=input_info.patch_length,
                in_chans=input_info.in_chans,
                embed_dim=embed_dim,
            )
            self.latent_size = input_info.video_size // input_info.patch_size
            self.latent_length = input_info.video_length // input_info.patch_length
        else:
            raise ValueError(f"Unsupported input type: {type(input_info)}")
...
class ViTDecoder(nn.Module):
    ...
    def __init__(
        ...
        assert conv_last_layer, "Only support conv_last_layer=True"
        ...
        if self.use_final_proj:
            ...
            self.final_proj = nn.Linear(
                embed_dim, self.unpatch_channels * (patch_size * patch_size * patch_length), bias=True
            )
            self.final_norm = norm_layer(self.unpatch_channels * (patch_size * patch_size * patch_length))

        self.last_layer = nn.Conv3d(
            in_channels=self.unpatch_channels, out_channels=3, kernel_size=3, stride=1, padding=1
        )
    ...
    def forward(self, x):
        ...
        if self.use_final_proj:
            ...
            x = self.final_norm(x)
        x = x.reshape(
            B, latentT, latentH, latentW, self.patch_length, self.patch_size, self.patch_size, self.unpatch_channels
        )
