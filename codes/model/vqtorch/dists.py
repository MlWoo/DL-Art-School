...
def check_shape(tensor, codebook):
    ...
    if tensor.size(2) != codebook.size(1):
        raise RuntimeError(
            "expected tensor and codebook to have the same feature "
...
def euclidean_distance(z, z_q):
    ...
    """
    if z.size() != z_q.size():
        raise RuntimeError("expected z and z_q to have the same shape but got " f"{z.size()} vs {z_q.size()}")

    z, z_q = z.reshape(z.size(0), -1), z_q.reshape(z_q.size(0), -1)
    return ((z_q - z) ** 2).mean(1)  # .sqrt()


def euclidean_cdist_topk(tensor, codebook, compute_chunk_size=1024, topk=1, half_precision=False):
    """
    ...
    return_dict = {"d": d.to(tensor_dtype).reshape(b, n, -1), "q": q.long().reshape(b, n, -1)}
    return return_dict
