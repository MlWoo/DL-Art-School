...
if __name__ == "__main__":
    """
    Ad-hoc script (hard coded; no command-line parameters) that spawns multiple separate trainers from a single optionss
    file, with a hard-coded set of modifications.
    """
    base_opt = "../experiments/sweep_music_mel2vec.yml"
    modifications = {
        "baseline": {},
        "lr1e3": {"steps": {"generator": {"optimizer_params": {"lr": {0.001}}}}},
        "lr1e5": {"steps": {"generator": {"optimizer_params": {"lr": {0.00001}}}}},
        "no_warmup": {"train": {"warmup_steps": 0}},
    }
    base_rank = 4
    opt = options.parse(base_opt, is_train=True)
    all_opts = []
