...
if not hasattr(os, "register_at_fork"):  # Windows and friends.
    ...
    os.register_at_fork(
        before=_acquireLock, after_in_child=_after_at_fork_child_reinit_locks, after_in_parent=_releaseLock
    )
...
class Handler(Filterer):
    ...
    def emit(self, record, flush_right_now=True):
        """
    ...
    def handle(self, record, flush_right_now=True):
        """
        ...
        if rv:
            self.acquire()
            try:
                self.emit(record, flush_right_now)
            finally:
                self.release()
        return rv
...
class FileHandler(StreamHandler):
    ...
    def __init__(self, filename, mode="a", encoding=None, delay=False):
        ...
        self.baseFilename = mfs.abspath(filename)
        self.mode = mode
    ...
    def _open(self):
        ...
        """
        assert self.encoding is None or self.encoding == "utf-8" or self.encoding == "bin" or self.encoding == "binary"
        if self.encoding == "bin" or self.encoding == "binary":
            mode = self.mode + "b"
            return GFile(self.baseFilename, mode, delay_flush=True)
        else:
            mode = self.mode
            return GFile(self.baseFilename, mode, delay_flush=True)
    ...
    def emit(self, record, flush_right_now=True):
        ...
        StreamHandler.emit(self, record, flush_right_now)
...
class Logger(Filterer):
    ...
    def _log(self, level, msg, args, exc_info=None, extra=None, stack_info=False, flush_right_now=True):
        """
        ...
        self.handle(record, flush_right_now)
    ...
    def handle(self, record, flush_right_now=True):
        ...
        if (not self.disabled) and self.filter(record):
            self.callHandlers(record, flush_right_now)
    ...
    def callHandlers(self, record, flush_right_now=True):
        """
        ...
        while c:
            for hdlr in c.handlers:
                ...
                if record.levelno >= hdlr.level:
                    hdlr.handle(record, flush_right_now)
            if not c.propagate:
                c = None  # break out
            else:
                c = c.parent
