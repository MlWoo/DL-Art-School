...
illuminants = {
    "A": {
        "2": torch.Tensor([(1.098466069456375, 1, 0.3558228003436005)]),
        "10": torch.Tensor([(1.111420406956693, 1, 0.3519978321919493)]),
    },
    "D50": {
        "2": torch.Tensor([(0.9642119944211994, 1, 0.8251882845188288)]),
        "10": torch.Tensor([(0.9672062750333777, 1, 0.8142801513128616)]),
    },
    "D55": {
        "2": torch.Tensor([(0.956797052643698, 1, 0.9214805860173273)]),
        "10": torch.Tensor([(0.9579665682254781, 1, 0.9092525159847462)]),
    },
    "D65": {
        "2": torch.Tensor([(0.95047, 1.0, 1.08883)]),  # This was: `lab_ref_white`
        "10": torch.Tensor([(0.94809667673716, 1, 1.0730513595166162)]),
    },
    "D75": {
        "2": torch.Tensor([(0.9497220898840717, 1, 1.226393520724154)]),
        "10": torch.Tensor([(0.9441713925645873, 1, 1.2064272211720228)]),
    },
    "E": {"2": torch.Tensor([(1.0, 1.0, 1.0)]), "10": torch.Tensor([(1.0, 1.0, 1.0)])},
}
...
def xyz2lab(xyz, illuminant="D65", observer="2"):
    ...
    xyzm = xyz.clone()
    xyz[~mask] = 7.787 * xyzm[~mask] + 16.0 / 116.0
...
def lab2xyz(lab, illuminant="D65", observer="2"):
    ...
    mask = out > 0.2068966
    outm = out.clone()
    out[mask] = torch.pow(outm[mask], 3.0)
    outm = out.clone()
    out[~mask] = (outm[~mask] - 16.0 / 116.0) / 7.787
